...
Five fix patterns, Android + iOS:
- **(a) Token storage:** EncryptedSharedPreferences/Keystore (Android) + iOS Keychain `kSecAttrAccessibleWhenUnlockedThisDeviceOnly` — never plain prefs/UserDefaults.
- **(b) Transport:** TLS 1.2+ with cert/public-key pinning via Network Security Config (Android) / ATS + `URLSession` trust check (iOS); no cleartext or trust-all.
- **(c) Exported component:** `android:exported="false"`, or signature-level permission if it must be callable.
- **(d) Deep link:** link only navigates; backend authorizes the action (session/entitlement/idempotency); verify host via assetlinks/AASA.
- **(e) WebView:** no JS bridge to untrusted content, file/universal access off, `MIXED_CONTENT_NEVER_ALLOW`, HTTPS only.

**Full Android + iOS vulnerable→fixed code:** see `references/vulnerable-fixed-examples.md`.
