...
Five before/after pairs cover the highest-leverage fixes:

- **(a) Dockerfile** — non-root `USER`, digest-pinned base, multi-stage distroless runtime, build-time secret mount (no baked token).
- **(b) Pod** — hardened `securityContext`: `runAsNonRoot`, `allowPrivilegeEscalation: false`, `drop: ["ALL"]`, read-only root FS, seccomp, resource limits.
- **(c) RBAC** — replace `cluster-admin` ClusterRoleBinding with a scoped namespaced `Role`/`RoleBinding` (explicit verbs/resources, no wildcards).
- **(d) Secrets** — `secretKeyRef` from a Secret object + etcd encryption-at-rest / external manager, instead of plaintext env `value:`.
- **(e) NetworkPolicy** — default-deny ingress/egress per namespace plus an explicit allow rule by pod label.

**Full per-language vulnerable→fixed code:** see `references/vulnerable-fixed-examples.md`.
