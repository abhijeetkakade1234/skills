...
The fixes across the per-language and gateway examples converge on the same moves:

- **(a) Per-key, per-tier limiter (Node):** key the limiter on the API key/client ID — not `req.ip` — and size the quota from the caller's plan tier.
- **(b) Distributed store (Python/Go):** replace in-memory/`memory://` limiters with a Redis-backed store keyed by API key so one quota is shared across instances/regions.
- **(c) Standard headers:** emit `RateLimit-Limit/Remaining/Reset` (+ legacy `X-RateLimit-*`) and always set `Retry-After` on 429 — never a bare 429/500.
- **(d) GraphQL:** add cost/complexity + depth-limit validation rules; reject over-budget nested/aliased queries before execution.
- **(e) Gateway usage plans:** AWS API Gateway usage plans (rate + burst + quota per key) and Kong `rate-limiting-advanced` (`identifier: consumer`, `strategy: redis`) enforce per-key quotas at the edge.

**Full per-language vulnerable→fixed code & gateway config:** see `references/vulnerable-fixed-examples.md`.
